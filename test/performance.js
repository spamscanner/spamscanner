...
describe('Performance Tests', () => {
	...
	test('performance: should maintain consistent performance', async () => {
		...
		assert.ok(average < 60_000); // Average should be under 60s
		assert.ok(maxDeviation < average * 10); // Very lenient for cold starts
	});
	...
	test('memory: should not leak memory with repeated scans', async () => {
		...
		assert.ok(memoryIncrease < 500); // Should not increase by more than 500MB
	});
});
