...
describe('Performance Benchmarks', () => {
	...
	test('benchmark: tokenization performance', async () => {
		...
		assert.ok(duration < 30_000); // Should complete within 30s
	...
	test('benchmark: basic email scanning', async () => {
		...
		assert.strictEqual(typeof result, 'object');
		assert.ok(duration < 60_000); // Should complete within 60s
	...
	test('benchmark: memory usage tracking', async () => {
		...
		// Memory increase threshold: 500MB (very lenient for different platforms)
		assert.ok(
			memoryIncrease < 500,
			`Memory increase (${memoryIncrease.toFixed(2)}MB) should be less than 500MB`,
		);
