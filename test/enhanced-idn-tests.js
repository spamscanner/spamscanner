...
// Shared scanner instance to avoid repeated TensorFlow model loading
let scanner;

// Enhanced IDN Homograph Attack Detection Tests
describe('Enhanced IDN Detection', () => {
	before(async () => {
		scanner = new SpamScanner();
		// Warm up the scanner by running a simple scan to load models
		await scanner.scan('warmup');
	});

	test('Basic homograph attacks', async () => {
	...
	test('Mixed script attacks', async () => {
	...
	test('Brand similarity protection', async () => {
	...
	test('Legitimate international domains', async () => {
...
WARNING: Failure to verify within 24 hours will result in permanent deletion.
	...
	test('Multiple domains in email', async () => {
...
And visit https://example.com for legitimate content.
	...
	test('Performance with large emails', async () => {
		...
		assert.ok(result.results.idnHomographAttack, 'Should have IDN detection results');
		// Allow 30s for processing (models already loaded in before hook)
		assert.ok(processingTime < 30_000, `Should process large email within reasonable time (took ${processingTime}ms)`);
